...
export function SidebarFooter({ theme: initialTheme }: { theme: string }) {
  ...
  return (
    <div className='mt-auto'>
      ...
      <div className='p-4 border-t dark:border-gray-700'>
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button
              variant='ghost'
              className='w-full justify-start px-2 gap-2 font-normal hover:bg-gray-200 dark:hover:bg-gray-700'
            >
              {(!mounted ? initialTheme : theme) === 'dark' ? (
                <Moon className='h-4 w-4' />
              ) : (
                <Sun className='h-4 w-4' />
              )}
              <span>
                {getThemeDisplay(!mounted ? initialTheme : theme) + ' Theme'}
              </span>
            </Button>
          </DropdownMenuTrigger>
          <DropdownMenuContent
      ...
      <Dialog
        ...
        onOpenChange={() => setShowClearAllDialog(false)}
      >
        <DialogContent className='bg-gray-100 dark:bg-gray-800 border-2 border-gray-200 dark:border-gray-700'>
          <DialogHeader>
            ...
            <DialogDescription className='text-gray-600 dark:text-gray-400'>
              Are you sure you want to delete all research reports? This action
              cannot be undone.
            </DialogDescription>
          </DialogHeader>
