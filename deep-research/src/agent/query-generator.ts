...
import { vercelGemini } from '../ai/providers';
...
import { z } from 'zod';
import { SerpQuery } from '../db/schema';
import { calculateQueriesAtDepth } from '@/utils/research';

export interface QueryWithObjectiveType {
    query: string;
    objective: string;
...
//     required: ['queries'],
// };
export class QueryWithObjectives {
    constructor(
        private wsManager: WebSocketManager
    ) { }

    async generateQueriesWithObjectives(
        researchId: string,
        parent_query_timestamp?: number
    ): Promise<SerpQuery[]> {
...
`
    ...
    Do not include any text outside the JSON object.
        ...
        `
        try {
            const { object: result } = await vercelGemini(
                {
                    system: systemPrompt,
                    user: userPrompt,
            ...
            // here we are creating a objective with needed information for a serp query.


            const serpQueries: SerpQuery[] = await Promise.all(result.queries.map(async (query, index: number) => {
                ...
                await this.wsManager.handleNewSerpQuery(researchId);
            ...
            // await db.updateSerpQueryResults(researchId, query.query_timestamp,
            //   query.parent_query_timestamp,
            //   failedQuery.successful_scraped_websites,
            //   'failed'
            // );
